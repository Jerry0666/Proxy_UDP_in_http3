...
type conf struct {
	ProxyServerIP   string `yaml:"proxyServerIP"`
	ProxyServerPort int    `yaml:"proxyServerPort"`
	PathIP1         string `yaml:"pathIP1"`
	PathPort1       int    `yaml:"pathPort1"`
	PathIP2         string `yaml:"pathIP2"`
	PathPort2       int    `yaml:"pathPort2"`
}

var c conf

func main() {
	yamlFile, err := os.ReadFile("../quicConfig.yaml")
	if err != nil {
	...
	fmt.Printf("config: %+v\n", c)
	IP := c.PathIP1 + ":" + strconv.Itoa(c.PathPort1)
	...
	path.SetIP(c.PathIP2, c.PathPort2)
...
func doProxyReq(client *http.Client, targetHost string, targetPort string) (*http.Response, error) {
	...
	URL += c.ProxyServerIP
	URL += ":"
	URL += strconv.Itoa(c.ProxyServerPort)
